...
const Home: React.FC = () => {
  ...
  // COLLAGE IMAGES (Updated from user request)
  const collageImages = [
  ...
  // Tripling the array to create a long scrollable buffer
  const displayImages = [...collageImages, ...collageImages, ...collageImages];
    ...
    const scroll = () => {
      if (scrollContainer && !isPaused) {
        if (scrollContainer.scrollLeft >= (scrollContainer.scrollWidth / 3) * 2) {
           // If we reached the end of the second set, snap back to start of second set to loop seamlessly
  ...
  return (
    <>
      ...
      {/* 5. SWIPEABLE IMAGE COLLAGE (Auto + Manual) */}
      <div className="py-8 bg-dark-900 border-y border-white/5">
        <div 
            ref={scrollRef}
            ...
            style={{ scrollBehavior: 'auto' }} // Ensure smooth manual scrolling
        >
            {displayImages.map((img, index) => (
            <div key={index} className="flex-shrink-0 w-[300px] h-[200px] relative">
                <img 
                    src={img} 
                    alt={`Gallery ${index}`} 
                    className="w-full h-full object-cover border-r border-dark-900/20"
                    draggable={false}
                />
            </div>
            ))}
        </div>
        <p className="text-center text-xs text-gray-600 mt-2">Swipe to view more</p>
      </div>
