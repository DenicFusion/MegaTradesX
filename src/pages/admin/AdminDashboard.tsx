...
import { Trash2, LogOut, Key, Plus, Image as ImageIcon, Upload, X, CheckCircle, ShieldCheck, Loader2 } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  // We now store the File object for Firebase, and a preview URL for the UI
  ...
  const handleAddImage = async (e: React.FormEvent) => {
    ...
    const result = await db.addImage(selectedFile, newImageDesc);
    
    if (result.success) {
      await refreshImages();
      setNewImageDesc('');
      clearSelection();
      setSuccessMsg('Image uploaded successfully to Cloud Storage');
      setTimeout(() => setSuccessMsg(''), 3000);
    } else {
      setErrorMsg(result.error || 'Failed to upload');
    }
    setUploading(false);
  };

  const handleDelete = async (image: GalleryImage) => {
    if (confirm('Delete this image permanently from cloud storage?')) {
      // Optimistic update for UI feel
      setImages(prev => prev.filter(img => img.id !== image.id));
      
      const success = await db.deleteImage(image);
      if (!success) {
        // Revert if failed (or just show error, re-fetching is safer)
        refreshImages();
            ...
            <div className="lg:col-span-1">
              <div className="glass-panel p-6 rounded-2xl sticky top-24">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2 text-white">
                  <Plus size={20} className="text-secondary" /> Add Image to Firebase
                </h3>
                ...
                <form onSubmit={handleAddImage} className="space-y-5">
                  ...
                  >
                    ...
                    {previewUrl ? (
                        ...
                        <div className="flex flex-col items-center gap-2 text-gray-400">
                            ...
                            <span className="text-[10px] text-gray-600">Supports JPG, PNG</span>
                        </div>
                    )}
                  </div>
                  ...
                  <button 
                    type="submit" 
                    disabled={uploading}
                    className="w-full bg-secondary hover:bg-blue-600 text-white py-3 rounded-xl text-sm font-bold shadow-lg shadow-blue-900/20 transition-all transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {uploading ? (
                      <>
